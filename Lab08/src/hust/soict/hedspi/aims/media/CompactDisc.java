...
public class CompactDisc extends Disc implements Playable{
	...
	public CompactDisc(String title, float cost, int length) {
	...
	public void addTrack(Track titleTrack) {
		if(this.tracks.contains(titleTrack) == true) {
			...
			System.out.println("Da track them thanh cong !");
		}
	}
	...
	public int compareTo(Object obj) {
		if(obj instanceof CompactDisc) {
			CompactDisc another = (CompactDisc) obj;
			if(this.tracks.size() < another.tracks.size()) return -1;
			else if(this.tracks.size() > another.tracks.size()) return 1;
			else {
				if(this.getLength() < another.getLength()) return -1;
				else if(this.getLength() > another.getLength()) return 1;
				else return 0;
			}
		}
		return 0;
